...
abstract class AbstractEndlessCommand extends Command
{
    ...
    /** @var int<0,max> $timeoutMicroseconds */
    private int $timeoutMicroseconds;

    /** @var int<0,max> $returnCode */
    ...
    public function __construct(?string $name = null)
    {
        ...
        $this->setTimeout(static::DEFAULT_TIMEOUT_SECONDS);
     ...
     *
     * @param float $timeoutMicroseconds Timeout between two iterations in seconds
     *
     * @throws InvalidArgumentException
     */
    public function setTimeout(float $timeoutMicroseconds): self
    {
        ...
        /** @var int<0,max> $timeoutMicroseconds */
        $timeoutMicroseconds = (int) (1000000.0 * $timeoutMicroseconds);
    ...
    {
        return ($this->timeoutMicroseconds / 1000000);
    }
    ...
    private function getMemoryInfo(bool $peak = false): array
    {
        ...
        $info['diffPercentage'] = ($lastUsage == 0) ? 0 : (float) $info['diff'] / (float) ($lastUsage / 100);
     ...
     */
    abstract protected function finalize(InputInterface $input, OutputInterface $output): void;
